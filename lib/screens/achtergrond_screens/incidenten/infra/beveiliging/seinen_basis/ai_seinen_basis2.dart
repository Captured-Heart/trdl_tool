          ...
          PopupMenuButton<WhereToGoFromAISeinenBasis2>(
            ...
            onSelected: (WhereToGoFromAISeinenBasis2 result) async {
              if (result == WhereToGoFromAISeinenBasis2.home_screen) {
                ...
                (result == WhereToGoFromAISeinenBasis2.ai_incidenten_basis) {
                await Navigator.pushNamed(context, 'ai_incidenten_basis');
              } if else {
                (result == WhereToGoFromAISeinenBasis2.ai_seinen_basis1) {
                await Navigator.pushNamed(context, 'ai_seinen_basis1');
              } if else {
                (result == WhereToGoFromAISeinenBasis2.ai_beveiliging_basis1) {
                await Navigator.pushNamed(context, 'ai_beveiliging_basis1');
              } if else {
                (result == WhereToGoFromAISeinenBasis2.ai_beveiliging_basis2) {
                await Navigator.pushNamed(context, 'ai_beveiliging_basis2');
              } if else {
                (result == WhereToGoFromAISeinenBasis2.ai_tegen_rijrichting) {
                await Navigator.pushNamed(context, 'ai_tegen_rijrichting');
              } if else {
                (result == WhereToGoFromAISeinenBasis2.ai_veiligheidsstoring_sein) {
                await Navigator.pushNamed(context, 'ai_veiligheidsstoring_sein');
              } else {
